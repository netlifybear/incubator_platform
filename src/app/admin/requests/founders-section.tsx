import { listFoundersWithBadges } from "@/lib/nominations";
...
export async function AdminFoundersBadgesSection({
  ...
  const founders = await listFoundersWithBadges(cohortId);
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl leading-7 text-[var(--muted)]">
            See which badges each founder holds.
          </p>
        </div>
      </div>
          ...
          {founders.map((founder) => (
            ...
            >
              ...
              {founder.badges.length === 0 ? (
                <p className="mt-2 text-xs text-[var(--muted)]">No badges yet</p>
              ) : (
                <div className="mt-3 flex flex-wrap gap-2">
                  {founder.badges.map((badge) => {
                    const il = issuerLabel(badge.issuerType);
                    const ic = issuerColor(badge.issuerType);
                    return (
                      <span
                        key={badge.type}
                        title={(badge.description ?? badge.def?.description ?? badge.type) + (il ? ` [${il === "V" ? "Vendor" : il === "I" ? "Investor" : "Admin"}]` : "")}
                        className="inline-flex items-center gap-1 rounded-full bg-[var(--panel-strong)] px-2.5 py-1 text-xs font-medium"
                      >
                        {badge.def?.icon ?? "\uD83C\uDFC6"} {badge.def?.label ?? badge.type}
